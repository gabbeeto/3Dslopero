...
function DisplayOn3D({
  ...
  let container: xyzNumbers[] = getContainersOfPairs(
    textGraph,
    Number(amountOfVarialbe),
  );
  generateValues(container);

  let [xOverworldLines, yOverworldLines, zOverworldLines] =
...
function makeLinesForXYZPoints(container: xyzNumbers[]): THREE.Line {
  ...
  let pointsForLine = container.map((e) => {
...
function MakeXYZPoints({
  ...
  return container.map((e, i) => {
    ...
    let remainingAxisAreZero =
      e[remaningAxis[0]] == 0 && e[remaningAxis[1]] == 0;
    ...
    return (
      <mesh
        ...
        key={i}
        position={[Number(e["x"]), Number(e["z"]), Number(e["y"])]}
      >
